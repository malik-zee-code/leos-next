...
const CommandBar = ({
  ...
  return (
    <Popover.Root open={open} onOpenChange={onOpenChange} defaultOpen={defaultOpen}>
      <Popover.Anchor className={cx("fixed inset-x-0 bottom-8 mx-auto flex w-fit items-center")} />
      <Popover.Portal>
        <Popover.Content
          side="top"
          sideOffset={0}
...
const CommandBarValue = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<"div">>(
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cx("px-3 py-2.5 text-sm tabular-nums text-gray-300", className)}
        {...props}
      />
    );
  }
...
>(({ className, ...props }, ref) => {
  return <div ref={ref} className={cx("h-4 w-px bg-gray-700", className)} {...props} />;
});
