...
const ChartLegend = (
  ...
  const filteredPayload = payload.filter((item: { type: string }) => item.type !== "none");

  return (
    <div ref={legendRef} className="flex items-center justify-end">
      <Legend
        categories={filteredPayload.map((entry: { value: string }) => entry.value)}
        colors={filteredPayload
          .map((entry: { value: string }) => categoryColors.get(entry.value))
...
const OverviewChartTooltip = ({
  ...
  if (active && payload) {
    const filteredPayload = payload.filter(
      (item: { value: number; name: string; payload: Record<string, unknown> }) =>
    ...
    const evolution = payload[0]?.payload.evolution as number;
            ...
            (
              ...
              return (
                <ChartTooltipRow
                  ...
                  name={
                    index === 0
                      ? (payloadData.formattedDate as string)
                      : (payloadData.previousFormattedDate as string)
                  }
...
const LineChart = React.forwardRef<HTMLDivElement, LineChartProps>((props, ref) => {
  ...
  function onDotClick(
    itemData: { index: number; dataKey: string; payload: Record<string, unknown> },
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          <XAxis
            ...
            ticks={
              startEndOnly
                ? ([data[0][index], data[data.length - 1][index]] as string[])
                : undefined
            }
            fill=""
            stroke=""
